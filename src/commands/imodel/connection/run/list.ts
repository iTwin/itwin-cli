...
import { StorageRunsResponse } from "../../../../services/synchronization/models/storage-run-response.js";
...
export default class ConnectionRunsListed extends BaseCommand {
  ...
  public static examples = [
    ...
    {
      command: `<%= config.bin %> <%= command.id %> --connection-id MWplZe9Uf0iR1IDMqyOMLqBN0_wHEVBGg_CzJmXdmE4 --top 10`,
      description: "Example 2: Limit the results to 10 runs",
    },
    {
      command: `<%= config.bin %> <%= command.id %> --connection-id MWplZe9Uf0iR1IDMqyOMLqBN0_wHEVBGg_CzJmXdmE4 --skip 5`,
  ...
  public async run(): Promise<StorageRunsResponse> {
    ...
    const result = await synchronizationApiService.getConnectionRuns(flags["connection-id"], flags.skip, flags.top);
