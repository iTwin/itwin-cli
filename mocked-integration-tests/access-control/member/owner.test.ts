...
const tests = () =>
  describe("Access Control Member Owner Tests", () => {
    const iTwinId = crypto.randomUUID();
    const memberId = crypto.randomUUID();

    it("Should remove owner member from iTwin", async () => {
      AccessControlApiStubber.removeiTwinOwnerMember.success(iTwinId, memberId);
    ...
    it("Should return an error when owner member is not found", async () => {
      AccessControlApiStubber.removeiTwinOwnerMember.memberNotFound(iTwinId, memberId);
    ...
    it("Should return an error when iTwin is not found", async () => {
      AccessControlApiStubber.removeiTwinOwnerMember.iTwinNotFound(iTwinId, memberId);
