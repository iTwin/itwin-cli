...
const tests = () =>
  describe("info", () => {
    ...
    it("should get user member", async () => {
      ...
      const { result: infoResult } = await runCommand<ResultResponse>(`access-control member user info --itwin-id ${iTwinId} --member-id ${memberId}`);
    ...
    it("should return an error when provided iTwin is not found", async () => {
      ...
      const { error: infoError } = await runCommand<ResultResponse>(`access-control member user info --itwin-id ${iTwinId} --member-id ${memberId}`);
