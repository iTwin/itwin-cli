...
import { AddedUserMembersResponse, UserMember } from "../../../src/services/access-control/models/user-member";
...
const tests = () => {
  ...
  it("Should display owner info of an iTwin in member info", async () => {
    ...
    const { result: getMemberUserInfo } = await runCommand<UserMember>(`access-control member user info --itwin-id ${iTwinId} --member-id ${userInfo!.id}`);
  ...
  it("Should add a new role to a user.", async () => {
    ...
    const { result: usersInfo } = await runCommand<UserMember[]>(`access-control member user list --itwin-id ${iTwinId}`);
    expect(usersInfo).to.not.be.undefined;
    expect(usersInfo).to.have.lengthOf(1);
    expect(usersInfo![0].roles).to.have.lengthOf(1);

    const { result: updateMemberResult } = await runCommand<UserMember>(
      `access-control member user update --itwin-id ${iTwinId} --member-id ${usersInfo![0].id} --role-id ${usersInfo![0].roles[0].id} --role-id ${newRole!.id}`,
    );
  ...
  it("Should add multiple users to iTwin using `--group-id` and `--role-ids` flags and delete them", async () => {
    ...
    const { result: createResult } =
      await runCommand<AddedUserMembersResponse>(`access-control member user add --itwin-id ${iTwinId} --email ${usersInfo[0].email} --email ${usersInfo[1].email} --email ${usersInfo[2].email}
  ...
  it("Should add multiple users to iTwin using `--group-id` and `--role-ids` (single list) flags and delete them", async () => {
    ...
    const { result: createResult } = await runCommand<AddedUserMembersResponse>(
      `access-control member user add --itwin-id ${iTwinId} --email ${usersInfo[0].email} --email ${usersInfo[1].email} --email ${usersInfo[2].email} --role-ids ${roleIds.join(",")}`,
    );
    expect(createResult).to.not.be.undefined;
    expect(createResult!.invitations).to.have.lengthOf(3);
    for (const [i, userInfo] of usersInfo.entries()) {
  ...
  it("Should fail to update iTwin group, when there are too many roles assigned", async () => {
    ...
    const result = await runCommand<AddedUserMembersResponse>(command);
  ...
  it("Should return an error when invalid uuid is provided as --itwin-id", async () => {
    const { error: addError } = await runCommand<UserMember>(
