...
const tests = () => {
  ...
  it("Should display owner info of an iTwin in member info", async () => {
    ...
    const { result: getMemberUserInfo } = await runCommand<Member>(
      `access-control member user info --itwin-id ${iTwinId} --member-id ${userInfo!.id}`
    );
  ...
  it("Should add a new role to a user.", async () => {
    ...
    const { result: updateMemberResult } = await runCommand<Member>(
  ...
  it("Should add multiple users to iTwin using `--group-id` and `--role-ids` flags and delete them", async () => {
    ...
    const { result: createResult } =
      await runCommand<MembersResponse>(`access-control member user add --itwin-id ${iTwinId} --email ${usersInfo[0].email} --email ${usersInfo[1].email} --email ${usersInfo[2].email}
  ...
  it("Should add multiple users to iTwin using `--group-id` and `--role-ids` (single list) flags and delete them", async () => {
    ...
    const { result: createResult } = await runCommand<MembersResponse>(
  ...
  it("should return an error when there are invalid UUIDs provided to `--role-ids` flag", async () => {
    ...
    const { error: createError } =
      await runCommand<MembersResponse>(`access-control member user add --itwin-id ${iTwinId} --email ${usersInfo[0].email}
