...
const tests = () => {
    ...
    it('Should create, update and list info and delete member group', async () => {
        ...
        const { result: createResult } = await runCommand<GroupMemberInfo[]>(`access-control member group add --itwin-id ${iTwinId} --groups ${serializedGroupsInfo}`);
        ...
        const { result: additionalRole } = await runCommand<Role>(`access-control role create -i ${iTwinId} -n "One More Test Role" -d "One More Test Role Description"`);
        expect(additionalRole).to.not.be.undefined;
        expect(additionalRole!.id).to.not.be.undefined;

        const { result: updateResult } = await runCommand<GroupMemberInfo>(`access-control member group update --itwin-id ${iTwinId} --group-id ${groupId1} --role-id ${roleId1} --role-id ${additionalRole!.id}`);
    ...
    it('Should create multiple member groups using `--groups` flag and delete them', async () => {
        ...
        const { result: createResult } = await runCommand<GroupMemberInfo[]>(`access-control member group add --itwin-id ${iTwinId} --groups ${JSON.stringify(groupsInfo)}`);
        expect(createResult).to.not.be.undefined;
    ...
    it('Should create multiple member groups using `--group-id` and `--role-ids` flags and delete them', async () => {
        ...
        const { result: resultCreate } = await runCommand<GroupMemberInfo[]>(`access-control member group add --itwin-id ${iTwinId} --group-id ${groupIds[0]} --group-id ${groupIds[1]} --group-id ${groupIds[2]} --role-ids ${roleIds.join(',')}`);
    ...
    it('Should return an error when there are too many role assignments', async () => {
        ...
        const { error: createError } = await runCommand<GroupMemberInfo[]>(`access-control member group add --itwin-id ${iTwinId} --groups ${serializedGroupsInfo}`);
        expect(createError).to.not.be.undefined;
