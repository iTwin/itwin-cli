...
import { SourceFile } from "../../../src/services/synchronization/models/source-file";
...
const tests = () =>
  describe("sourcefile", () => {
    ...
    it("should list sourcefiles", async () => {
      ...
      expect(
        listResult?.some((result) => result.id === addResult1!.id && result.storageFileId === addResult1!.storageFileId && result!.connectorType === "IFC"),
      ).to.be.true;
      expect(
