...
const tests = () =>
  describe("sourcefile", () => {
    ...
    it("should add/get/delete a sourcefile", async () => {
      const { result: addResult } = await runCommand<SourceFile>(
        `imodel connection sourcefile add -c ${connectionId} --connector-type MSTN --storage-file-id ${anotherTestFileId}`
      );
      ...
      const { result: infoResult } = await runCommand<SourceFile>(
        `imodel connection sourcefile info -c ${connectionId} --source-file-id ${addResult?.id}`
      );
    ...
    it("should update connector-type and storage-file-id of a sourcefile", async () => {
      const { result: addResult } = await runCommand<SourceFile>(
        `imodel connection sourcefile add -c ${connectionId} --connector-type IFC --storage-file-id ${anotherTestFileId}`
      );
      expect(addResult).to.not.be.undefined;
      expect(addResult!.connectorType).to.be.equal("IFC");
      expect(addResult!.storageFileId).to.be.equal(anotherTestFileId);

      const { result: infoResult1 } = await runCommand<SourceFile>(
        `imodel connection sourcefile info -c ${connectionId} --source-file-id ${addResult?.id}`
      );
      ...
      const { result: updateResult } = await runCommand<SourceFile>(
        `imodel connection sourcefile update -c ${connectionId} --source-file-id ${addResult?.id} --connector-type MSTN --storage-file-id ${yetAnotherTestFileId}`
      );
      ...
      const { result: infoResult2 } = await runCommand<SourceFile>(
        `imodel connection sourcefile info -c ${connectionId} --source-file-id ${addResult?.id}`
      );
    ...
    it("should list sourcefiles", async () => {
      const { result: addResult1 } = await runCommand<SourceFile>(
        `imodel connection sourcefile add -c ${connectionId} --connector-type IFC --storage-file-id ${anotherTestFileId}`
      );
      ...
      const { result: addResult2 } = await runCommand<SourceFile>(
        `imodel connection sourcefile add -c ${connectionId} --connector-type MSTN --storage-file-id ${yetAnotherTestFileId}`
      );
      ...
      expect(
        listResult?.some(
          (result) => result.id === addResult1!.id && result.storageFileId === addResult1!.storageFileId && result!.connectorType === "IFC"
        )
      ).to.be.true;
      expect(
        listResult?.some(
          (result) => result.id === addResult2!.id && result.storageFileId === addResult2!.storageFileId && result!.connectorType === "MSTN"
        )
      ).to.be.true;
